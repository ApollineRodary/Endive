...
fn js_tm_to_kernel_tm(value: &JsValue) -> Result<Tm, JsValue> {
    ...
    match tag.as_str() {
        ...
        "abstraction" => {
            let bound_ty = js_tm_to_kernel_tm(&Reflect::get(value, &JsValue::from_str("type"))?)?;
...
fn kernel_tm_to_js_tm(tm: &Tm) -> JsValue {
    match tm {
        ...
        Tm::Abs(binding) => {
            ...
            Reflect::set(&obj, &JsValue::from_str("bound_ty"), &kernel_tm_to_js_tm(&binding.bound_ty)).unwrap();
