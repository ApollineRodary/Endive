...
})({"1YyS6":[function(require,module,exports) {
...
/*import { endiveGenerator } from "./generators/endive.js";*/ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "resize", ()=>resize);
var _latexJs = require("./generators/latex.js");
var _htmlJs = require("./generators/html.js");
var _verifyJs = require("./verify.js");
var _examplesJs = require("./examples.js");
...
function updateMathDisplay() {
    if (globalThis.compileLatex) {
        const htmlCode = (0, _htmlJs.htmlGenerator).workspaceToCode(workspace);
        ...
        const latexCode = (0, _latexJs.latexGenerator).workspaceToCode(workspace);
...
const verifyProofs = function(button) {
    console.log((0, _verifyJs.validate)(workspace));
};
let toolbox = {
...
for (const [key, value] of Object.entries(defaultVariableNames))workspace.createVariable(key, value);
...
function toggleLatexCompil() {
    if (globalThis.compileLatex) document.getElementById("toggleLatexCompil").classList.remove("pressed");
    else document.getElementById("toggleLatexCompil").classList.add("pressed");
...
function load_example(name) {
    Blockly.serialization.workspaces.load(JSON.parse((0, _examplesJs.examples)[name]), workspace);
    console.log("Hey");
}
document.getElementById("toggleMathDisplay").addEventListener("click", toggleMathDisplay);
document.getElementById("verifyProof").addEventListener("click", verifyProofs);
document.getElementById("toggleAutomaticVerif").addEventListener("click", toggleautomatic);
document.getElementById("toggleLatexCompil").addEventListener("click", toggleLatexCompil);
document.getElementById("saveWorkspace").addEventListener("click", saveWorkspace);
for(let name in 0, _examplesJs.examples){
    document.getElementById("example-" + name).addEventListener("click", ()=>load_example(name));
    console.log(name);
}
/*
endiveGenerator.init(workspace);
latexGenerator.init(workspace);
*/ (0, _htmlJs.htmlGenerator).init(workspace);

},{"./verify.js":"k3MTU","./generators/latex.js":"8jbPA","./generators/html.js":"aMOvR","./examples.js":"c1vsw","@parcel/transformer-js/src/esmodule-helpers.js":"5yOdK"}],"k3MTU":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "validate", ()=>validate);
var _endiveWasm = require("endive-wasm");
var _endiveWasmDefault = parcelHelpers.interopDefault(_endiveWasm);
var _endiveWasmBgWasm = require("url:endive-wasm/endive_wasm_bg.wasm");
var _endiveWasmBgWasmDefault = parcelHelpers.interopDefault(_endiveWasmBgWasm);
const _ = require("47ca597b8196046e");
...
function deBruijnIndex(environment, name) {
    ...
    if (index == -1) throw new Error("Asked for index of an unbound variable name");
    return environment.length - index - 1;
}
function lift(lterm) {
    if (lterm.type == "variable") return {
        type: "variable",
        index: lterm.index + 1
    };
    if (lterm.type == "abstraction" || lterm.type == "pi") return {
        type: lterm.type,
...
function convertStatement(block, environment, types) {
    /** Converts a statement block into the corresponding lambda-term */ if (block == null) throw new Error("Incomplete statement");
    if (block.type === "proposition_forall") {
        let name = block.getField("VARIABLE").getText();
        let typeName = block.getField("TYPE").getText();
        if (!(typeName in types)) {
            block.setWarningText("Ce type n'est pas d\xe9fini.");
            throw new Error("Unbound type");
        }
        let type = types[typeName];
        let propositionBlock = block.getInput("PROPOSITION").connection.targetBlock();
        if (propositionBlock == null) {
            block.setWarningText("Ce quantificateur n'est pas complet.");
            throw new Error("Incomplete statement");
        }
        let proposition = convertStatement(propositionBlock, environment.concat([
            [
                name,
                type
            ]
    ...
    if (block.type === "proposition_implies") {
        ...
        if (antecedentBlock == null || consequentBlock == null) {
            block.setWarningText("Cette implication n'est pas compl\xe8te.");
            throw new Error("Incomplete statement");
        }
        let antecedent = convertStatement(antecedentBlock, environment, types);
        let consequent = convertStatement(consequentBlock, environment.concat([
            [
                makeFreshVariable(environment),
                antecedent
            ]
        ]).map((x)=>[
                x[0],
    ...
    if (block.type === "proposition_reference") {
        ...
        if (!isBound(environment, name)) {
            block.setWarningText("Cet \xe9l\xe9ment n'est pas d\xe9fini.");
...
function convertTactics(block, environment, hypotheses, types) {
   ...
   */ if (block == null) return hypotheses[hypotheses.length - 1][0];
    if (block.type === "tactic_let") {
        ...
        let type = types[typeName];
        let restOfProof = convertTactics(block.nextConnection.targetBlock(), environment.concat([
            [
                name,
                type
            ]
    ...
    if (block.type === "tactic_suppose") {
        ...
        if (hypothesisBlock == null) {
            block.setWarningText("Vous n'avez pas pr\xe9cis\xe9 d'\xe9nonc\xe9 pour cette hypoth\xe8se.");
            throw new Error("Missing hypothesis");
        }
        let hypothesis = convertStatement(hypothesisBlock, environment, types);
        let offsetHypothesis = lift(hypothesis);
        let hypothesisName = makeFreshVariable(environment);
        let restOfProof = convertTactics(block.nextConnection.targetBlock(), environment.map((x)=>[
                x[0],
                lift(x[1])
            ]).concat([
            [
    ...
    if (block.type === "tactic_then") {
        ...
        if (conclusionBlock == null) {
            block.setWarningText("Vous n'avez pas pr\xe9cis\xe9 d'\xe9nonc\xe9 pour cette conclusion.");
        ...
        let lterm = null;
        for(let i = 0; i < hypotheses.length; ++i)//if (endive.betaEquivalent(hypotheses[i][1], conclusion)) {
        ...
        for(let i = 0; i < hypotheses.length; ++i){
            if (lterm !== null) break;
            for(let j = 0; j < hypotheses.length; ++j)if (_.isEqual(hypotheses[i][1], {
                type: "pi",
        ...
        if (lterm !== null) return convertTactics(block.nextConnection.targetBlock(), environment, hypotheses.concat([
            [
                lterm,
                conclusion
            ]
        ]), types);
        block.setWarningText("Vos hypoth\xe8ses ne permettent pas d'arriver \xe0 cette conclusion.");
...
function getTopologicalOrder(dag) {
   ...
   */ let visited = {};
    ...
    function visit(vertex) {
        if (visited[vertex.name] == "visiting") throw new Error("Circular definition");
    ...
    dag.forEach((vertex)=>{
        if (!visited[vertex.name]) visit(vertex);
    });
    return order;
}
function verifyTheorem(block, types) {
    /* Reads a theorem block and verifies that it is valid and that the proof is correct */ // Read statement
    let statementBlock = block.getInput("STATEMENT").connection.targetBlock();
    if (statementBlock == null) {
        block.setWarningText("Vous n'avez pas pr\xe9cis\xe9 d'\xe9nonc\xe9 pour ce th\xe9or\xe8me.");
        return false;
    }
    let statement;
    try {
    ...
    try {
        let statementType = _endiveWasm.inferType(statement);
        if (statementType.type !== "universe") throw new Error("Not a universe type");
    } catch (e) {
        statementBlock.setWarningText("Cet \xe9nonc\xe9 est invalide :(");
        return false;
    }
    // Read proof
    let proofBlock = block.getInput("PROOF").connection.targetBlock();
    if (proofBlock == null) {
        block.setWarningText("Vous n'avez pas prouv\xe9 ce th\xe9or\xe8me.");
        return false;
    }
    let proof;
    try {
    ...
    let isBetaEquivalent = _endiveWasm.betaEquivalent(_endiveWasm.inferType(proof), statement);
    if (!isBetaEquivalent) {
        block.setWarningText("Cette preuve est valide, mais ne prouve pas ce que vous avez \xe9nonc\xe9 :(");
        return false;
    } else {
...
function getInductiveTypeDependencies(block) {
   ...
   */ const definitionBlocks = [
    ...
    if (block.type !== "definition_inductive_type") throw new Error("Not a definition block");
    let dependencies = [];
    let name = block.getField("NAME").getText();
    let constructorBlock = block.getInput("CONSTRUCTORS").connection.targetBlock();
    while(constructorBlock !== null){
        let constructorName;
        if (constructorBlock.type == "definition_arrow_constructor") {
            let parameterBlock = constructorBlock.getInput("PARAMETERS").connection.targetBlock();
            while(parameterBlock.type == "definition_arrow_param"){
                let parameterName = parameterBlock.getField("PARAMETER").getText();
                if (parameterName !== name && !dependencies.includes(parameterName)) dependencies.push(parameterName);
                parameterBlock = parameterBlock.getInput("NEXT").connection.targetBlock();
            }
        }
...
function registerInductiveTypeDefinition(block, types, constructors) {
    ...
    if (typeName in types || typeName in constructors) {
        block.setWarningText(`${typeName} est d\xe9j\xe0 d\xe9fini`);
    ...
    while(constructorBlock !== null){
        let constructorBody = {};
        let constructorName;
        if (constructorBlock.type == "definition_simple_constructor") constructorBody = {
            type: "variable",
            index: 0
        };
            ...
            if (parameterBlock == null) {
                constructorBlock.setWarningText("Il manque un param\xe8tre pour ce constructeur.");
                throw new Error("Missing parameter for constructor");
            }
            while(parameterBlock.type == "definition_arrow_param"){
                let parameterName = parameterBlock.getField("PARAMETER").getText();
                if (parameterName == typeName) constructorBody = {
                    type: "pi",
                    variable: {
                        type: "variable",
                        index: 0
                ...
                else if (parameterName in types) constructorBody = {
                    type: "pi",
                    variable: types[parameterName],
                    body: lift(constructorBody)
                };
                else {
                    parameterBlock.setWarningText(`Le type ${parameterName} n'est pas d\xe9fini.`);
                    throw new Error("Unbound type in constructor parameter");
                }
                parameterBlock = parameterBlock.getInput("NEXT").connection.targetBlock();
            }
        } else {
            constructorBlock.setWarningText("Seuls les constructeurs sont autoris\xe9s ici.");
        ...
        if (constructorName in types || constructorName in constructors) {
            constructorBlock.setWarningText(`${constructorName} est d\xe9j\xe0 d\xe9fini.`);
...
async function validate(workspace) {
   ...
   */ await (0, _endiveWasmDefault.default)((0, _endiveWasmBgWasmDefault.default));
    ...
    try {
        ...
        for(let i = 0; i < blocks.length; ++i){
            let block = blocks[i];
            if (block.type.startsWith("definition")) block.setWarningText("L'espace de travail inclut des d\xe9finitions cycliques.");
        }
    }
    ...
    inductiveTypeDefinitionOrder.forEach(function(definitionName) {
        let block = blocks.find((bl)=>bl.type == "definition_inductive_type" && bl.getField("NAME").getText() == definitionName);
        if (block) registerInductiveTypeDefinition(block, types, constructors);
    });
    // Verify theorems
    for(let i = 0; i < blocks.length; ++i){
        let block = blocks[i];
        if (block.type == "theorem") {
            ...
            if (verifyTheorem(block, types)) addGlow(block, "#00ff00");
            else addGlow(block, "#bb0000");
        }
    }
}

},{"endive-wasm":"c5agd","url:endive-wasm/endive_wasm_bg.wasm":"lceBS","@parcel/transformer-js/src/esmodule-helpers.js":"5yOdK","47ca597b8196046e":"8Mljs"}],"c5agd":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 * @param {any} n
 ...
 * @param {any} m
 * @returns {any}
 */ parcelHelpers.export(exports, "betaEquivalent", ()=>betaEquivalent);
...
async function __wbg_init(input) {
    if (wasm !== undefined) return wasm;
    if (typeof input === "undefined") input = new URL(require("8ac6324ea9dca2ce"));
    const imports = __wbg_get_imports();
...
},{"8ac6324ea9dca2ce":"d9LsM","@parcel/transformer-js/src/esmodule-helpers.js":"5yOdK"}],"d9LsM":[function(require,module,exports) {
module.exports = require("c706c5b4adc0c7ff").getBundleURL("7JE76") + "endive_wasm_bg.9b7d4a3c.wasm" + "?" + Date.now();

},{"c706c5b4adc0c7ff":"jZtML"}],"jZtML":[function(require,module,exports) {
"use strict";
var bundleURL = {};
...
},{}],"5yOdK":[function(require,module,exports) {
...
},{}],"lceBS":[function(require,module,exports) {
module.exports = require("939d7c64cf5cf538").getBundleURL("7JE76") + "endive_wasm_bg.02b0a675.wasm" + "?" + Date.now();

},{"939d7c64cf5cf538":"jZtML"}],"8Mljs":[function(require,module,exports) {
var global = arguments[3];
(function() {
...
},{}],"8jbPA":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "latexGenerator", ()=>latexGenerator);
const latexGenerator = new Blockly.Generator("LaTeX");
...
latexGenerator["proposition_forall"] = function(block, generator) {
    const variable = latexGenerator.getVariableName(block.getFieldValue("VARIABLE"));
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"5yOdK"}],"aMOvR":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "htmlGenerator", ()=>htmlGenerator);
const htmlGenerator = new Blockly.Generator("HTML");
htmlGenerator.RESERVED_WORDS_ = "Th\xe9or\xe8me,Preuve";
...
htmlGenerator["theorem"] = function(block, generator) {
    ...
    const code = `<b>Th\xe9or\xe8me :</b> $${statement}$ <br/><br/> <i>Preuve :</i> <br/> ${proof} <br/>`;
    return code;
};
htmlGenerator["proposition_forall"] = function(block, generator) {
    const variable = htmlGenerator.getVariableName(block.getFieldValue("VARIABLE"));
    const type = htmlGenerator.getVariableName(block.getFieldValue("TYPE"));
    const proposition = generator.valueToCode(block, "PROPOSITION", 0);
    return [
        `\\forall ${variable} \\in ${type}, ${proposition}`,
        0
    ];
};
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"5yOdK"}],"c1vsw":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "examples", ()=>examples);
const examples = {
    identity: '{"blocks":{"languageVersion":0,"blocks":[{"type":"theorem","id":"fnrz.,2wTO{#q;*cm{TG","x":-240,"y":-222,"inputs":{"STATEMENT":{"block":{"type":"proposition_forall","id":"eIRCG3MZU2%Kc!W~@saY","fields":{"VARIABLE":{"id":"c2@r(?c|`z5iamv6,@B{"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"inputs":{"PROPOSITION":{"block":{"type":"proposition_implies","id":"8f}/]wnyR8-_8O5rp;Yc","inputs":{"ANTECEDENT":{"block":{"type":"proposition_reference","id":"0g65VFI^+CqQbisoo-@E","fields":{"NAME":{"id":"c2@r(?c|`z5iamv6,@B{"}}}},"CONSEQUENT":{"block":{"type":"proposition_reference","id":"7bAq}#B3iDRjqUzN!j(H","fields":{"NAME":{"id":"c2@r(?c|`z5iamv6,@B{"}}}}}}}}}},"PROOF":{"block":{"type":"tactic_let","id":"6=8b/J,4:WsNZo!CqOrx","fields":{"VARIABLE":{"id":"c2@r(?c|`z5iamv6,@B{"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"next":{"block":{"type":"tactic_suppose","id":"mTByb/Q1.k4_tuya/{Et","inputs":{"HYPOTHESIS":{"block":{"type":"proposition_reference","id":"-HjEB:(RPYAE)SmdXus2","fields":{"NAME":{"id":"c2@r(?c|`z5iamv6,@B{"}}}}},"next":{"block":{"type":"tactic_then","id":"n[Qx8c4vPzw%pAGb2Keu","inputs":{"CONCLUSION":{"block":{"type":"proposition_reference","id":"CzsVuIW8MT`mrmBqs7,:","fields":{"NAME":{"id":"c2@r(?c|`z5iamv6,@B{"}}}}}}}}}}}}}]},"variables":[{"name":"x","id":"c2@r(?c|`z5iamv6,@B{","type":"MathObject"},{"name":"y","id":"KJkyO7^*StuMsE-xxN0x","type":"MathObject"},{"name":"z","id":"QICmv;@`-NGyb8`,|jX;","type":"MathObject"},{"name":"P","id":"jYd)VM=_aH,Rf9@|K{uB","type":"MathObject"},{"name":"Q","id":"Hk0*-f$B-a)Slyn^44q6","type":"MathObject"},{"name":"Prop","id":"#-PcBLXw[91QNuo~uR4!","type":"Type"}]}',
    transitivity: '{"blocks":{"languageVersion":0,"blocks":[{"type":"theorem","id":"v5n@9i%!Xqw6`0Jr.o7b","x":-513,"y":-332,"inputs":{"STATEMENT":{"block":{"type":"proposition_forall","id":"m3cj%a{*hh%Kq[5ZMMUM","fields":{"VARIABLE":{"id":"jYd)VM=_aH,Rf9@|K{uB"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"inputs":{"PROPOSITION":{"block":{"type":"proposition_forall","id":"0E2x9trV-YZdo[IKToYV","fields":{"VARIABLE":{"id":"Hk0*-f$B-a)Slyn^44q6"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"inputs":{"PROPOSITION":{"block":{"type":"proposition_forall","id":"2Z~dGsg9VgfWn~5uH+=B","fields":{"VARIABLE":{"id":"QICmv;@`-NGyb8`,|jX;"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"inputs":{"PROPOSITION":{"block":{"type":"proposition_implies","id":"UrdO94W}dZ57_.b/ye).","inline":false,"inputs":{"ANTECEDENT":{"block":{"type":"proposition_implies","id":"dv|s%(fei_NM(R=8fG?{","inputs":{"ANTECEDENT":{"block":{"type":"proposition_reference","id":"=b:1+fyC:9FLOzfM|P1^","fields":{"NAME":{"id":"jYd)VM=_aH,Rf9@|K{uB"}}}},"CONSEQUENT":{"block":{"type":"proposition_reference","id":"$sMqZrT|H9KNKFt:R3?#","fields":{"NAME":{"id":"Hk0*-f$B-a)Slyn^44q6"}}}}}}},"CONSEQUENT":{"block":{"type":"proposition_implies","id":"=0#J=oGDCY+Yu36G.jSs","inline":false,"inputs":{"ANTECEDENT":{"block":{"type":"proposition_implies","id":"]ZPg#MY^3ww:7Bvn9F)y","inputs":{"ANTECEDENT":{"block":{"type":"proposition_reference","id":"492xg[(fY6n^{@/TU2^j","fields":{"NAME":{"id":"Hk0*-f$B-a)Slyn^44q6"}}}},"CONSEQUENT":{"block":{"type":"proposition_reference","id":"n%12vsR^ECU#jJ~dr%Iv","fields":{"NAME":{"id":"QICmv;@`-NGyb8`,|jX;"}}}}}}},"CONSEQUENT":{"block":{"type":"proposition_implies","id":"K_Kp5+LaEiFvgE=SktiA","inputs":{"ANTECEDENT":{"block":{"type":"proposition_reference","id":"J+a!xR_G]9B0-Xh[u,+#","fields":{"NAME":{"id":"jYd)VM=_aH,Rf9@|K{uB"}}}},"CONSEQUENT":{"block":{"type":"proposition_reference","id":"_*2HMLm|31)~7N]mkH1n","fields":{"NAME":{"id":"QICmv;@`-NGyb8`,|jX;"}}}}}}}}}}}}}}}}}}}}}},"PROOF":{"block":{"type":"tactic_let","id":"@Bm]n4e`90D57O^zUS}W","fields":{"VARIABLE":{"id":"jYd)VM=_aH,Rf9@|K{uB"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"next":{"block":{"type":"tactic_let","id":":,=:?gd`_ZrM_V%8m^ls","fields":{"VARIABLE":{"id":"Hk0*-f$B-a)Slyn^44q6"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"next":{"block":{"type":"tactic_let","id":"4Gm+4yyCKv`X|y{_}8r5","fields":{"VARIABLE":{"id":"QICmv;@`-NGyb8`,|jX;"},"TYPE":{"id":"#-PcBLXw[91QNuo~uR4!"}},"next":{"block":{"type":"tactic_suppose","id":"{TsRsPG;:)]x1;hi9j*C","inputs":{"HYPOTHESIS":{"block":{"type":"proposition_implies","id":"NYq[IO-c0}D.Y]m?qw-E","inputs":{"ANTECEDENT":{"block":{"type":"proposition_reference","id":";~7QiTwcw_f~8yE=l0%=","fields":{"NAME":{"id":"jYd)VM=_aH,Rf9@|K{uB"}}}},"CONSEQUENT":{"block":{"type":"proposition_reference","id":"gO%B#NgiYPi/hC?{JI9Q","fields":{"NAME":{"id":"Hk0*-f$B-a)Slyn^44q6"}}}}}}}},"next":{"block":{"type":"tactic_suppose","id":";q1(Iq!je]}Ja%?4KPr7","inputs":{"HYPOTHESIS":{"block":{"type":"proposition_implies","id":"N[h{t,RYUrrK!Q4kh]e}","inputs":{"ANTECEDENT":{"block":{"type":"proposition_reference","id":"*.k(5n%j3N7wE6G5=a-H","fields":{"NAME":{"id":"Hk0*-f$B-a)Slyn^44q6"}}}},"CONSEQUENT":{"block":{"type":"proposition_reference","id":"_[*+Q.{Oe6aYfoVtGm~X","fields":{"NAME":{"id":"QICmv;@`-NGyb8`,|jX;"}}}}}}}},"next":{"block":{"type":"tactic_suppose","id":"kkzrsBQOPF9byW~oq`Vl","inputs":{"HYPOTHESIS":{"block":{"type":"proposition_reference","id":"[mW]#*5em(7nG_F[s7Iu","fields":{"NAME":{"id":"jYd)VM=_aH,Rf9@|K{uB"}}}}},"next":{"block":{"type":"tactic_then","id":"c0OptiG3[!csfBQaMwX|","inputs":{"CONCLUSION":{"block":{"type":"proposition_reference","id":"_!%VxB8rq};Lio5#YiuI","fields":{"NAME":{"id":"Hk0*-f$B-a)Slyn^44q6"}}}}},"next":{"block":{"type":"tactic_then","id":".e/^jv[])M$FcJ))NlGK","inputs":{"CONCLUSION":{"block":{"type":"proposition_reference","id":"qi#5B*Qy^9.7x{DO?o?h","fields":{"NAME":{"id":"QICmv;@`-NGyb8`,|jX;"}}}}}}}}}}}}}}}}}}}}}}}]},"variables":[{"name":"x","id":"c2@r(?c|`z5iamv6,@B{","type":"MathObject"},{"name":"y","id":"KJkyO7^*StuMsE-xxN0x","type":"MathObject"},{"name":"R","id":"QICmv;@`-NGyb8`,|jX;","type":"MathObject"},{"name":"P","id":"jYd)VM=_aH,Rf9@|K{uB","type":"MathObject"},{"name":"Q","id":"Hk0*-f$B-a)Slyn^44q6","type":"MathObject"},{"name":"Prop","id":"#-PcBLXw[91QNuo~uR4!","type":"Type"}]}',
    inductive: '{"blocks":{"languageVersion":0,"blocks":[{"type":"definition_inductive_type","id":"Jq_/=]!318zKVurMgO.]","x":-401,"y":-297,"fields":{"NAME":{"id":"T[|b?*9y^hi/KkLA^UuT"}},"inputs":{"CONSTRUCTORS":{"block":{"type":"definition_simple_constructor","id":"KQ!m`NH+1Px{/OM7q-Fa","fields":{"NAME":{"id":"~}gOD.xAEc#+%]LaYfl?"}},"next":{"block":{"type":"definition_arrow_constructor","id":"38]V4kX:dWwIXrgco;mD","fields":{"NAME":{"id":"4BDz/f{p_}#YEfMNv-hn"}},"inputs":{"PARAMETERS":{"block":{"type":"definition_arrow_param","id":"?=s{cY]A8mRJ_|@aVuq{","fields":{"PARAMETER":{"id":"T[|b?*9y^hi/KkLA^UuT"}},"inputs":{"NEXT":{"block":{"type":"definition_arrow_end","id":"C{d;6n,%sEWfO3u)Y-hc"}}}}}}}}}}}},{"type":"definition_unary_predicate","id":"zGI7++XMp=Aph-q,U!`w","x":-402,"y":-154,"fields":{"NAME":{"id":"Bp%bD|7RK8)vfv~XO}F?"},"TYPE":{"id":"T[|b?*9y^hi/KkLA^UuT"}},"inputs":{"RULES":{"block":{"type":"definition_predicate_rule","id":"`aYYWXb~DyR`7EZ+Jl.-","fields":{"NAME":{"id":"K+uqr2({1~|c#OhM.E)K"}},"inputs":{"PROPOSITION":{"block":{"type":"definition_unary_predicate_true","id":"]ki52hne;O~(]TG.T`?g","inputs":{"VALUE":{"block":{"type":"constructor_simple_final","id":"|oo%;3x^K]m~qY%z|A`H","fields":{"NAME":{"id":"~}gOD.xAEc#+%]LaYfl?"}}}}}}}},"next":{"block":{"type":"definition_predicate_rule","id":"kug6cGWTW7,9_UkYK$v}","fields":{"NAME":{"id":"MCYe+DDz~]-gglbWWK)E"}},"inputs":{"PROPOSITION":{"block":{"type":"definition_predicate_forall","id":"RV}+xGaX8aN+RYD3zff,","fields":{"VARIABLE":{"id":"c2@r(?c|`z5iamv6,@B{"},"TYPE":{"id":"T[|b?*9y^hi/KkLA^UuT"}},"inputs":{"PROPOSITION":{"block":{"type":"definition_predicate_implies","id":",c,h(@ySp)Kp2D0{8DQl","inputs":{"ANTECEDENT":{"block":{"type":"definition_unary_predicate_true","id":"%`H(VCGPg3N2M(K+^c3/","inputs":{"VALUE":{"block":{"type":"constructor_simple_final","id":"k%#p|O%oirw+B/gCJM|Y","fields":{"NAME":{"id":"c2@r(?c|`z5iamv6,@B{"}}}}}}},"CONSEQUENT":{"block":{"type":"definition_unary_predicate_true","id":"txr]KWBRLq#8v]jm8hL4","inputs":{"VALUE":{"block":{"type":"constructor_arrow_final","id":":2$XTDC#)c~O:w8:/T5[","fields":{"NAME":{"id":"4BDz/f{p_}#YEfMNv-hn"}},"inputs":{"BODY":{"block":{"type":"constructor_arrow_final","id":"/dM7eGPt#O[]W,Oc.4Qc","fields":{"NAME":{"id":"4BDz/f{p_}#YEfMNv-hn"}},"inputs":{"BODY":{"block":{"type":"constructor_simple_final","id":"4Jwff|Z.boZ6fK8C=K!y","fields":{"NAME":{"id":"c2@r(?c|`z5iamv6,@B{"}}}}}}}}}}}}}}}}}}}}}}}}}}]},"variables":[{"name":"x","id":"c2@r(?c|`z5iamv6,@B{","type":"MathObject"},{"name":"y","id":"KJkyO7^*StuMsE-xxN0x","type":"MathObject"},{"name":"R","id":"QICmv;@`-NGyb8`,|jX;","type":"MathObject"},{"name":"P","id":"jYd)VM=_aH,Rf9@|K{uB","type":"MathObject"},{"name":"Q","id":"Hk0*-f$B-a)Slyn^44q6","type":"MathObject"},{"name":"Prop","id":"#-PcBLXw[91QNuo~uR4!","type":"Type"},{"name":"N","id":"T[|b?*9y^hi/KkLA^UuT","type":"Type"},{"name":"i","id":"K?vgyc?GVOoW}Jz4GENY","type":"Type"},{"name":"pair","id":"Bp%bD|7RK8)vfv~XO}F?","type":"Predicate"},{"name":"zero pair","id":"K+uqr2({1~|c#OhM.E)K","type":"PredicateRule"},{"name":"plus deux","id":"MCYe+DDz~]-gglbWWK)E","type":"PredicateRule"},{"name":"zero","id":"~}gOD.xAEc#+%]LaYfl?","type":"Constructor"},{"name":"S","id":"4BDz/f{p_}#YEfMNv-hn","type":"Constructor"},{"name":"j","id":"~2.[I6-FO@%.#Vx|R)`u","type":"Constructor"},{"name":"k","id":"cjC@Zn}81[A]Xzh=X-Mr","type":"Constructor"},{"name":"m","id":"K]dHyP0mgtM16qQ?+h0!","type":"Constructor"}]}'
};

},{"@parcel/transformer-js/src/esmodule-helpers.js":"5yOdK"}]},["1YyS6","3cYfC"], "3cYfC", "parcelRequireae47")
